...
def save_to_db(df: pd.DataFrame):
    ...
    rows.to_sql("vibration", con, if_exists="append", index=False)
    con.close()
...
if uploaded:
    ...
    if not df_new.empty:
        save_to_db(df_new)
        st.success(f"✅ {len(df_new)} baris data berhasil disimpan.")

df_hist = load_history()

# Gunakan data terbaru (dari upload atau histori)
if not df_new.empty:
    df_work = df_new.copy()
...
with tab4:
    ...
    if df_hist.empty:
        ...
        st.divider()
        if st.button("🗑️ Hapus Semua Data Historis", type="secondary"):
            con = sqlite3.connect(DB_PATH)
