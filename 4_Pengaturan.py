...
from utils import load_history, get_zone, get_threshold, THRESHOLD, add_zone_cols

st.set_page_config(page_title="Trend Vibrasi — PLTU TBK", page_icon="📈", layout="wide")
st.markdown("""
<style>
...
if df_hist.empty:
    st.info("📂 Belum ada data. Upload file Excel dari halaman Ringkasan.")
    st.stop()
...
if mode == "Detail satu equipment":
    ...
    for d in sel_dir:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="lines+markers", name=f"Direction {d}",
            line=dict(color=colors_dir.get(d,"#888"), width=2),
            marker=dict(size=7),
            hovertemplate=f"<b>Direction {d}</b><br>%{{x|%d-%b-%Y}}<br>%{{y:.3f}} mm/s<extra></extra>",
        ))
    ...
    all_eq = sorted(df_hist["equipment"].unique())
    cols_per_row = 2
    rows = [all_eq[i:i+cols_per_row] for i in range(0, len(all_eq), cols_per_row)]
    for row_equips in rows:
        cols = st.columns(cols_per_row)
        for col, eq in zip(cols, row_equips):
            with col:
                thr = get_threshold(eq)
                df_eq = df_hist[df_hist["equipment"]==eq].sort_values("date")

                # Tabel nilai terbaru
                latest_eq = df_eq.groupby(["titik","direction"], as_index=False).last()[["titik","direction","value"]]
                ...
                for i, titik in enumerate(sorted(df_eq["titik"].unique())):
                    for d in ["H","V","A"]:
                        ...
                        fig_eq.add_trace(go.Scatter(
                            ...
                            line=dict(color=colors_dir.get(d,"#888"), width=1.5, dash=ls_list[i%4]),
                            marker=dict(size=5),
                        ))
                ...
                fig_eq.update_layout(
                    ...
                    yaxis_title="mm/s", showlegend=True,
