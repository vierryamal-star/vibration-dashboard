...
# ─── Topbar ────────────────────────────────────────────────────────────────────
df_hist_pre = load_history()
zone_d_count = 0
zone_c_count = 0
if not df_hist_pre.empty:
    _tmp = df_hist_pre.copy()
    _tmp["value"] = pd.to_numeric(_tmp["value"], errors="coerce")
    _latest = _tmp.sort_values("date").groupby(["unit","equipment","titik","direction"], as_index=False).last()
    for _, r in _latest.iterrows():
        thr_t = "Turbine" if "turbine" in str(r["equipment"]).lower() else "Pump/Fan"
        z = get_zone(r["value"], THRESHOLD[thr_t])[0]
        if z == "ZONE D": zone_d_count += 1
        elif z == "ZONE C": zone_c_count += 1

col_hdr, col_alm = st.columns([5, 2])
with col_hdr:
    st.markdown("### ⚡ Dashboard Monitoring Vibrasi — PLTU TBK")
    st.caption("ISO 10816 · Vibrasi dalam mm/s RMS")
with col_alm:
    if zone_d_count > 0:
        st.error(f"🔴 {zone_d_count} Zone D aktif")
    elif zone_c_count > 0:
        st.warning(f"🟠 {zone_c_count} Zone C aktif")
    else:
        st.success("✅ Semua titik normal")
    if uploaded and not df_new.empty:
        st.caption(f"📂 Upload terakhir: {datetime.now().strftime('%d %b %Y %H:%M')}")
...
# TAB
# ═══════════════════════════════════════════════════════════════════════════════
# Hitung badge alarm untuk label tab
_d = sum(1 for _, r in df_filtered.apply(lambda r: get_zone(r["value"], THRESHOLD["Turbine" if "turbine" in str(r["equipment"]).lower() else "Pump/Fan"])[0], axis=1).items() if r == "ZONE D") if not df_filtered.empty else 0
_alarm_label = f"🚨 Alarm ({_d})" if _d > 0 else "🚨 Alarm"
...
# ─────────────────────────────────────────────────────────────────────────────
with tab5:
    st.subheader("⚙️ Pengaturan Threshold ISO 10816")
    st.caption("Nilai threshold default sesuai ISO 10816. Ubah sesuai kebutuhan PLTU TBK.")

    col_s1, col_s2 = st.columns(2)
    with col_s1:
        st.markdown("**Turbine (mm/s RMS)**")
        ta = st.number_input("Zone A ≤", value=float(THRESHOLD["Turbine"]["A"]), step=0.1, key="s_ta")
        tb = st.number_input("Zone B ≤", value=float(THRESHOLD["Turbine"]["B"]), step=0.1, key="s_tb")
        tc = st.number_input("Zone C ≤", value=float(THRESHOLD["Turbine"]["C"]), step=0.1, key="s_tc")
        st.caption("Di atas Zone C → Zone D (bahaya)")
    with col_s2:
        st.markdown("**Pump / Fan (mm/s RMS)**")
        pa = st.number_input("Zone A ≤", value=float(THRESHOLD["Pump/Fan"]["A"]), step=0.1, key="s_pa")
        pb = st.number_input("Zone B ≤", value=float(THRESHOLD["Pump/Fan"]["B"]), step=0.1, key="s_pb")
        pc = st.number_input("Zone C ≤", value=float(THRESHOLD["Pump/Fan"]["C"]), step=0.1, key="s_pc")
        st.caption("Di atas Zone C → Zone D (bahaya)")

    if st.button("💾 Simpan Threshold", type="primary"):
        THRESHOLD["Turbine"]  = {"A": ta, "B": tb, "C": tc}
        THRESHOLD["Pump/Fan"] = {"A": pa, "B": pb, "C": pc}
        st.success("✅ Threshold berhasil diperbarui.")

    st.divider()
    st.subheader("📋 Legenda Zone ISO 10816")
...
| Item | Keterangan |
|------|-----------|
| Standar | ISO 10816 — Mechanical vibration |
| Satuan | mm/s RMS |
| Sumber data | Upload Excel manual (.xlsx) |
| Penyimpanan | SQLite lokal (vibration_history.db) |
| Threshold Turbine | Zone A: 3.8 / Zone B: 7.5 / Zone C: 11.8 |
| Threshold Pump/Fan | Zone A: 1.4 / Zone B: 2.8 / Zone C: 4.5 |
    """)
