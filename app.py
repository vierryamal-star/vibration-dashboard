...
def dir_block(label, val, thr):
    if val is None or pd.isna(val):
        return f'<div style="flex:1;text-align:center;background:var(--color-background-tertiary);border-radius:6px;padding:5px 2px"><div style="font-size:15px;color:var(--color-text-secondary)">{label}</div><div style="font-size:12px;color:var(--color-text-secondary)">–</div></div>'
    zk2 = get_zone(val, thr)[0]
    c2  = CBORDER.get(zk2,"#94a3b8")
    return f'<div style="flex:1;text-align:center;background:var(--color-background-tertiary);border-radius:6px;padding:5px 2px"><div style="font-size:15px;color:var(--color-text-secondary)">{label}</div><div style="font-size:12px;font-weight:500;color:{c2}">{val:.3f}</div></div>'
