...
df_hist["date"]  = pd.to_datetime(df_hist["date"],  errors="coerce", utc=False)
df_hist["value"] = pd.to_numeric(df_hist["value"],  errors="coerce")
# Normalisasi: simpan kolom date_only sebagai string YYYY-MM-DD untuk perbandingan aman
df_hist["date_only"] = df_hist["date"].dt.normalize().dt.strftime("%Y-%m-%d")
...
if len(date_range) == 2:
    ...
    df_show = df_hist[
        (df_hist["date_only"] >= d_from) &
        (df_hist["date_only"] <= d_to) &
...
with del_tab1:
    ...
    if sel_dates:
        mask = df_hist["date_only"].isin(sel_dates)
        ...
        summary = df_preview.groupby("date_only").size().reset_index()
