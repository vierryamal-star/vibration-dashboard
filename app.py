...
with st.sidebar:
    ...
    if not df_hist.empty:
        ...
        nd = sum(1 for _, r in lc.iterrows()
                 if get_zone(r["value"], THRESHOLD[
                     "Turbine" if "turbine" in str(r["equipment"]).lower() else "Pump/Fan"
                 ])[0] == "ZONE D")
...
# ── Buang baris value non-numerik (antisipasi data kotor dari DB) ─────────────
df_hist = df_hist[df_hist["value"].notna()].copy()
...
sel_unit_btn = st.radio("Unit", unit_opts, horizontal=True, key="mon_unit",
...
def _safe_float(val):
    """
    Konversi val ke float dengan aman.
    Mengembalikan None jika val bukan angka (termasuk string HTML).
    """
    if val is None:
        return None
    try:
...
def dir_block(label, val, titik, thr):
    ...
    val    : nilai vibrasi — di-cast ke float secara aman dulu
    titik  : nama lokasi titik ukur tertinggi (mis. 'DE Motor', 'NDE Pump')
    """
    # ── Guard: pastikan val benar-benar float, bukan string/HTML ─────────────
    val = _safe_float(val)
    ...
    if val is None:
        ...
        ">
            ...
            <div style="font-size:13px;color:var(--color-text-secondary);
                        margin:2px 0">–</div>
            <div style="font-size:9px;color:var(--color-text-secondary)">–</div>
        </div>
        '''

    zk2 = get_zone(val, thr)[0]
    c2  = CBORDER.get(zk2, "#94a3b8")
    titik_display = titik if len(str(titik)) <= 18 else str(titik)[:16] + "…"
...
# Card base — semua direction (tanpa filter direction)
...
for eq in sorted(df_card_latest["equipment"].dropna().unique()):
    ...
    def max_dir_info(d, _df=df_eq):
        sub = _df[(_df["direction"] == d) & (_df["value"].notna())]
        if sub.empty:
            return None, None
        row = sub.loc[sub["value"].idxmax()]
        return _safe_float(row["value"]), str(row["titik"])

    h_val, h_titik = max_dir_info("H")
    v_val, v_titik = max_dir_info("V")
    a_val, a_titik = max_dir_info("A")

    all_vals = [v for v in [h_val, v_val, a_val] if v is not None]
...
">
  <div style="display:flex;justify-content:space-between;
    ...
    <div style="font-size:13px;font-weight:500;
                color:var(--color-text-primary)">{r['eq']}</div>
...
if df_d.empty and df_c.empty:
    ...
    if not df_d.empty:
        ...
        sd = df_d[["unit", "equipment", "titik", "direction", "value",
                   "zone_label", "date"]].copy()
        ...
        sd = sd.rename(columns={
            ...
            "direction": "Dir", "value": "mm/s", "zone_label": "Status",
            "date": "Tanggal"
        })
    ...
    if not df_c.empty:
        ...
        sc = df_c[["unit", "equipment", "titik", "direction", "value",
                   "zone_label", "date"]].copy()
...
sel_det  = st.selectbox("Pilih Equipment", sorted(latest["equipment"].unique()),
                        key="mon_det")
thr_det  = get_threshold(sel_det)
df_det   = latest[latest["equipment"] == sel_det].copy().sort_values(
    ["titik", "direction"])
...
pivot_det["Max (mm/s)"] = pivot_det["Max (mm/s)"].map(
