...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    dir_valid   = [d for d in sel_dir_p
                   if len(df_sel[df_sel["direction"]==d]) >= 3]
    ...
    if not dir_valid:
        st.warning("Data historis terlalu sedikit (minimum 3 titik per direction) "
                   "untuk semua direction yang dipilih.")
        st.stop()

    if dir_skipped:
        st.info(f"ℹ️ Direction **{', '.join(dir_skipped)}** dilewati "
                f"karena data < 3 titik dalam rentang yang dipilih.")
